...
for file in files:
    ...
    # CSVファイル読み込み
    csv_input = pd.read_csv(filepath_or_buffer="C:\MorinoFolder\Python\KabuRadar\CSV\entry\TimeChart" + csv_code + ".csv", encoding="ms932", sep=",")
    ...
    for i, row in csv_input.iterrows():
        ...
        cur = str(int(row["終値"]))
        vol = str(int(row["出来高"]))
        tpl = (dt, csv_code, opn, hi, low, cur, vol)
